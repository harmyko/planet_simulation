...
Planet::Planet()
{
    impl = std::make_unique<PlanetImpl>("Unnamed", 0, 0, 0, 0);
}

Planet::Planet(std::string name, double mass, double radius, double distance, double velocity)
{
    impl = std::make_unique<PlanetImpl>(name, mass, radius, distance, velocity);
}

Planet::Planet(const Planet &other)
{
    impl = std::make_unique<PlanetImpl>(*other.impl);
}
